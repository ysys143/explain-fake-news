...
# torchtext imdb 대신 kagglehub에서 IMDB 파일 다운로드
import pandas as pd
...
class IMDBDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        text = row["review"]
        label = 0 if row["sentiment"] == "negative" else 1  # Convert to 0/1
        return text, label
...
def get_imdb_dataset(split="test"):
    ...
    # Create IMDBDataset instance
    return IMDBDataset(data=imdb_data, split=split)
